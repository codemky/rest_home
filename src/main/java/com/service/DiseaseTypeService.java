package com.service;

import com.entity.DiseaseType;
import java.util.List;

/**
 * (DiseaseType)表服务接口
 *
 * @author makejava
 * @since 2020-04-19 19:30:57
 */
public interface DiseaseTypeService {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    DiseaseType queryById(Integer id);

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<DiseaseType> queryAllByLimit(int offset, int limit);
    
    /**
     * 根据实例对象查询数据
     * @param diseaseType 实例对象
     * @return 对象列表
     */
    List<DiseaseType> queryAllByDiseaseType(DiseaseType diseaseType);
    
    /**
     * 查询所有数据
     *
     * @return 对象列表
     */
    public List<DiseaseType> queryAll();

    /**
     * 新增数据
     *
     * @param diseaseType 实例对象
     * @return 实例对象
     */
    DiseaseType insert(DiseaseType diseaseType);

    /**
     * 修改数据
     *
     * @param diseaseType 实例对象
     * @return 更新记录数
     */
    int update(DiseaseType diseaseType);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 更新记录数
     */
    int deleteById(Integer id);

}