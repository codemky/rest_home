...
public class MyMD5 {
     ...
     * @apiNote 输入密码明文，输出带盐密文
     */
    public static String MD5WithSalt(String inputStr) {
        String salt = salt();

//        System.out.println("从Hash提取的盐值为" + salt);
        String hashResult = MD5(inputStr + salt);//加盐，输入加盐 哈希计算,转换输出
//        System.out.println("加盐密文：" + hashResult);
     ...
     * @return: salt
     * @params:
     * @apiNote 自定义简单生成盐，是一个随机生成的长度为16的字符串，每一个字符是随机的十六进制字符
     */
     ...
     * @apiNote: 从库中查找到的hash值提取出的salt
     */
    public static String getHashFromSaltHash(String hash) {
